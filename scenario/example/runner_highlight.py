...
try:
    sys.path.append('/home/labor/eclipse-workspace/transaid/sumo/tools/')
...
# POI id representing the RSU
RSU_IDS = ["rsu5000", "rsu5001", "rsu5002"]

# interval for sending CAMs (just visualization)
CAM_INTERVAL = 4.0
DENM_INTERVAL = 4.0
RUN_BASELINE = False
...
def run(downwardEdgeID, distance):
    ...
    """
    global NOAD_ZONE_ENTRY_POS, LAST_DENM
    ...
    # Add POI representing RSU
    for i, rsuID in enumerate(RSU_IDS):
        traci.poi.add(rsuID, 300 + i*800, 10, [255]*4, poiType="rsu", layer=1, imgFile="RSU.jpeg", width=10, height=10)
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        allVehs = []
        for plt in platoons.values():
            allVehs.extend([v for _, v in plt.getVehicles()])
        allVehs.extend(pendingToCVehs)
        for veh in allVehs:
            if veh.lastCAM < t - CAM_INTERVAL:
                veh.lastCAM = t
                traci.vehicle.highlight(vehID, (0, 0, 255, 255), 2.5, 200, 2, 1)
                if veh.lane in ["e0_0", "e0_1"]:
                    if veh.pos < 250:
                        traci.poi.highlight(RSU_IDS[0], (0, 0, 255, 255), 5, 200, 2, 1)
                    elif veh.pos < 1050 and veh.pos > 750:
                        traci.poi.highlight(RSU_IDS[1], (0, 0, 255, 255), 5, 200, 2, 1)
                    elif veh.pos < 1850 and veh.pos > 1550:
                        traci.poi.highlight(RSU_IDS[2], (0, 0, 255, 255), 5, 200, 2, 1)
                
        # DENM highlighting
        if t % DENM_INTERVAL == 0:
            for rsuID in RSU_IDS:
                traci.poi.highlight(rsuID, (0, 255, 255, 255), 6.5, 200, 2, 2)
            for veh in allVehs:                
                if veh.lane in ["e0_0", "e0_1"]:
                    if random.random() > 0.25:
                        if veh.pos < 250 or (veh.pos < 1050 and veh.pos > 750) or (veh.pos < 1850 and veh.pos > 1550):
                            traci.vehicle.highlight(veh.ID, (0, 255, 255, 255), 4, 200, 2, 2)
...
def requestToC(vehID, timeUntilMRM):
    ...
    # MCM highlighting
    traci.vehicle.highlight(vehID, (255, 0, 0, 255), 5.5, 200, 2, 3)    
    pos = traci.vehicle.getLanePosition(vehID)
    if pos < 500:
        traci.poi.highlight(RSU_IDS[0], (255, 0, 0, 255), 8, 200, 2, 3)
    elif pos < 1300 and pos > 500:
        traci.poi.highlight(RSU_IDS[1], (255, 0, 0, 255), 8, 200, 2, 3)
    elif pos < 2100 and pos > 1300:
        traci.poi.highlight(RSU_IDS[2], (255, 0, 0, 255), 8, 200, 2, 3)
...
#     "--queue-output", "outputQueue%s"%options.suffix])
    ...
    sumo_args = ["-c", "../sumo.cfg", "-r", "../routes_trafficMix_0_trafficDemand_1_driverBehaviour_OS_seed_0.xml", "-a", "../additionalsOutput_trafficMix_0_trafficDemand_1_driverBehaviour_OS_seed_0.xml, ../vTypesCAVToC_OS.add.xml, ../vTypesCVToC_OS.add.xml, ../vTypesLV_OS.add.xml, ../shapes.add.xml, ../view.add.xml"]
    
# Debug
#     port=12345
#     print ("Calling: %s"%reduce(lambda x,y: str(x)+" "+str(y), [sumoBinary] + sumo_args + ["--remote-port", str(port)]))
